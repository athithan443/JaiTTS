...
import tqdm
import argparse
from thirdparty.UniSpeech.downstreams.speaker_verification.verification import verification
import os
...
model = None
score_list = []
use_accelerator = args.device != "cpu"
for t1, t2 in tqdm.tqdm(zip(tsv1, tsv2), total=len(tsv1)):
    ...
    if not os.path.exists(t1_path) or not os.path.exists(t2_path):
        continue
    try:
        sim, model = verification(args.model_name, t1_path, t2_path, use_gpu=use_accelerator, checkpoint=args.checkpoint, wav1_start_sr=args.wav1_start_sr, wav2_start_sr=args.wav2_start_sr, wav1_end_sr=args.wav1_end_sr, wav2_end_sr=args.wav2_end_sr, model=model, wav2_cut_wav1=args.wav2_cut_wav1, device=args.device)
