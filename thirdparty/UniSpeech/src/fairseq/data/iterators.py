...
class EpochBatchIterator(EpochBatchIterating):
    ...
    def next_epoch_itr(
        ...
        if self._next_epoch_itr is not None:
            ...
            if hasattr(self.dataset, "reset_batch_sampler"):
                self._frozen_batches = self.dataset.reset_batch_sampler()
        ...
        if self._supports_prefetch:
            ...
            if shuffle and not fix_batches_to_gpus:
                if hasattr(self.dataset, 'shuffle_batches'):
                    batches = self.dataset.shuffle_batches(list(batches), self.seed + epoch)
                else:
            ...
            if shuffle and fix_batches_to_gpus:
                if hasattr(self.dataset, 'shuffle_batches'):
                    batches = self.dataset.shuffle_batches(list(batches), self.seed + epoch + self.shard_id)
                else:
                    batches = shuffle_batches(batches, self.seed + epoch + self.shard_id)
        else:
            if shuffle:
                if hasattr(self.dataset, 'shuffle_batches'):
                    batches = self.dataset.shuffle_batches(list(self.frozen_batches), self.seed + epoch)
                else:
...
class EpochPipeIterator(EpochBatchIterator):
    """A multi-epoch iterator over a :class:`torch.utils.data.Dataset`.
    ...
    def _get_iterator_for_epoch(
        ...
        if torch.distributed.is_initialized():
            self.dataset.reset(distributed_utils.get_data_parallel_world_size(), distributed_utils.get_data_parallel_rank())
        else:
            self.dataset.reset(1, 0)
        # Create data loader
        itr = torch.utils.data.DataLoader(
            self.dataset,
            batch_size=1,
            ...
            num_workers=self.num_workers,
            pin_memory=True
            #worker_init_fn=self.dataset.init_worker
        )
