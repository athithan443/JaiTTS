...
from thirdparty.UniSpeech.src.fairseq.binarizer import safe_readline
...
from thirdparty.UniSpeech.src.fairseq.tokenizer import tokenize_line
...
class Dictionary:
    ...
    def __init__(
        ...
        if input_file is not None and 'txt' in input_file:
            self.add_from_file(input_file)            
        else:
            pass
        if spm_model is not None:
            self.processor = spm.SentencePieceProcessor()
            self.processor.load(spm_model)
        else:
            self.processor = None
    ...
    def encode_line(
        ...
        if not isinstance(line, list):
            if self.processor is not None:
                words = self.processor.encode_as_pieces(line)
            else:
