...
from thirdparty.UniSpeech.src.fairseq.modules import gelu_accurate
import torch
import torch.nn.functional as F
from thirdparty.UniSpeech.src.fairseq.modules.multihead_attention import MultiheadAttention
...
def load_ensemble_for_inference(filenames, task, model_arg_overrides=None):
    from thirdparty.UniSpeech.src.fairseq import checkpoint_utils
...
def replace_unk(hypo_str, src_str, alignment, align_dict, unk):
    from thirdparty.UniSpeech.src.fairseq import tokenizer
        ...
        if module_path not in import_user_module.memo:
            ...
            if module_name not in sys.modules:
                ...
                if os.path.exists(tasks_path):
                    from thirdparty.UniSpeech.src.fairseq.tasks import import_tasks

                    import_tasks(tasks_path, f"{module_name}.tasks")

                models_path = os.path.join(module_path, "models")
                if os.path.exists(models_path):
                    from thirdparty.UniSpeech.src.fairseq.models import import_models
...
def get_perplexity(loss, round=2, base=2):
    from thirdparty.UniSpeech.src.fairseq.logging.meters import safe_round
